...
def mklink():
    ...
    symlink(target, link_, options.directory)
    sys.stdout.write("Symbolic link created: %(link)s --> %(target)s\n" % vars())


def _is_target_a_directory(link_, rel_target):
    """
    If creating a symlink from link to a target, determine if target
    is a directory (relative to dirname(link)).
    """
    target = os.path.join(os.path.dirname(link_), rel_target)
    return os.path.isdir(target)


def symlink(target, link_name, target_is_directory=False):
    """
    An implementation of os.symlink for Windows (Vista and greater)
    """
    target_is_directory = (target_is_directory or
        _is_target_a_directory(link_name, target))
    # normalize the target (MS symlinks don't respect forward slashes)
    target = os.path.normpath(target)
    handle_nonzero_success(api.CreateSymbolicLink(link_name, target, target_is_directory))


def link(target, link_):
    """
    Establishes a hard link between an existing file and a new file.
    """
    handle_nonzero_success(api.CreateHardLink(link_, target, None))
...
def trace_symlink_target(link_):
    """
    ...
    if not is_symlink(link_):
        raise ValueError("link must point to a symlink on the system")
    while is_symlink(link_):
        orig = os.path.dirname(link_)
        link_ = readlink(link_)
        link_ = resolve_path(link_, orig)
    return link_
...
def readlink(link_):
    ...
    if not rdb.tag == api.IO_REPARSE_TAG_SYMLINK:
        raise RuntimeError(
            "Expected IO_REPARSE_TAG_SYMLINK, but got %d" % rdb.tag
        )
