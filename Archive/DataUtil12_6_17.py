...
def get_frontier(df_data, acct, scope):
    '''
    *** Under Construction: returns a dataframe of four portfolios (max return, targ return, tangency, min var)
    ...
    returns = du.returnize0(df_data) # get daily returns from dataframe
    avg_rets = returns.mean() # get mean of all daily returns
    weights, exp_return, std_dev = optimize.portfolio_optimizer.target_opt( returns ) # optimize
    port1 = 'Max_return'
    df = df.append({'Portfolio': port1, 'exp_return': exp_return, 'std_dev': std_dev})
    ...
    port2 = 'Target_return'
    df = df.append({'Portfolio': port2, 'exp_return': exp_return, 'std_dev': std_dev})
